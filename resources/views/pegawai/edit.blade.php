...
<style>
    ...
    .preview-crop {
        ...
        display: flex; align-items: center; justify-content: center;
        color: #94a3b8;
        padding: 0; /* Diubah agar gambar full jika sudah ada */
    }
    /* Memastikan area potong di modal tetap lingkaran */
        ...
        <div class="col-md-10">
            ...
            <div class="d-flex flex-column flex-md-row justify-content-between align-items-md-center mb-4 gap-3">
                <div>
                    ...
                    <p class="text-muted mb-0">Perbarui informasi pegawai di bawah ini.</p>
                </div>
                ...
                <div class="card-body p-4 p-md-5">
                    ...
                    @if ($errors->any())
                        <div class="alert alert-danger shadow-sm border-0 rounded-4 mb-4 d-flex align-items-start p-4">
                            ...
                            <div>
                                <strong class="d-block mb-2 text-danger">Perhatian! Ada input yang kurang pas:</strong>
                    ...
                    <form action="{{ route('update-pegawai', ['id' => $pegawai->id]) }}" method="POST" enctype="multipart/form-data" onsubmit="return disableBtnSubmit(this)">
                        @csrf 
                        @method('PUT') 
                        ...
                        <div class="form-section">
                            ...
                            <div class="row mb-4">
                                <div class="col-md-12">
                                    ...
                                    <input type="text" name="nama" class="form-control fw-semibold text-dark" required value="{{ old('nama', $pegawai->nama) }}">
                                </div>
                            </div>
                            ...
                            <div class="row g-4 mb-4">
                                <div class="col-md-6">
                                    ...
                                    <div class="input-group shadow-sm rounded-3">
                                        ...
                                        <input type="text" name="no_hp" class="form-control border-start-0 ps-0 bg-white" placeholder="08..." value="{{ old('no_hp', $pegawai->no_hp) }}">
                                    </div>
                                </div>
                                <div class="col-md-6">
                                    ...
                                    <div class="input-group shadow-sm rounded-3">
                                        ...
                                        <input type="email" name="email" class="form-control border-start-0 ps-0 bg-white" value="{{ old('email', $pegawai->email) }}">
                                    </div>
                                </div>
                            </div>
                                ...
                                <div class="upload-box p-4 d-flex flex-column flex-sm-row align-items-center gap-4">
                                    <div class="preview-crop shadow-sm" id="box_preview">
                                        @if($pegawai->foto_profil)
                                            <img src="{{ asset('storage/' . $pegawai->foto_profil) }}" style="width:100%; height:100%; object-fit:cover;">
                                        @else
                                            <i class="fas fa-user text-opacity-50 text-secondary fs-1"></i>
                                        @endif
                                    </div>
                                    <div class="text-center text-sm-start flex-grow-1">
                                        <h6 class="fw-bold text-dark mb-1">Ganti Foto Wajah</h6>
                                        <p class="text-muted small mb-3">Format JPG/PNG. Lewati jika tidak ingin mengubah foto saat ini.</p>
                        ...
                        <div class="form-section">
                                ...
                                <div class="col-md-6">
                                    <div class="mb-4">
                                        ...
                                        <select name="jenis_pegawai" id="jenis_pegawai" class="form-select border-primary" required>
                                            ...
                                            <option value="PPPK" {{ old('jenis_pegawai', $pegawai->jenis_pegawai) == 'PPPK' ? 'selected' : '' }}>PPPK (Penuh Waktu)</option>
                                    ...
                                    <div id="wrapper_tmt_pengangkatan" class="reveal-wrapper">
                                        <div class="p-3 bg-primary bg-opacity-10 rounded-4 border border-primary border-opacity-25">
                                            <label class="form-label fw-bold text-primary small mb-1">TMT Pengangkatan (Awal Karir) <span class="text-danger d-none" id="req_tmt_pengangkatan">*</span></label>
                                    ...
                                    <div id="wrapper_sub_unit" class="reveal-wrapper">
                                        <div class="p-3 bg-light rounded-4 border">
                                            <label class="form-label fw-bold small text-secondary mb-2">
                                                <i class="fas fa-sitemap me-2 text-muted"></i>Pilih Sub Bagian:
                                            </label>
                            ...
                            <div id="wrapper_golongan" class="reveal-wrapper">
                                <div class="mb-1">
                                    ...
                                    <small class="text-muted mt-2 d-block">Sesuaikan jika ada kenaikan pangkat.</small>
                                </div>
                            </div>
                        </div>
                        ...
                        <div id="wrapper_tmt" class="reveal-wrapper">
                            ...
                            <div class="bg-info bg-opacity-10 border border-info border-opacity-25 rounded-4 p-3 d-flex align-items-center mb-4">
                                ...
                                <div>
                                    <strong class="text-dark d-block mb-1">Pengaturan Jadwal Kenaikan</strong>
                                    <span class="text-muted small">Update data ini hanya jika ada perubahan TMT (Kenaikan Pangkat/KGB). Sistem akan memprediksi jadwal selanjutnya berdasarkan tanggal ini.</span>
                                </div>
                            </div>
                                ...
                                <div class="col-md-6">
                                    <div class="p-3 bg-white border rounded-4 shadow-sm h-100">
                                        ...
                                        <input type="date" name="tmt_pangkat_terakhir" id="tmt_pangkat" class="form-control bg-light" 
                                               value="{{ old('tmt_pangkat_terakhir', $pegawai->tmt_pangkat_terakhir) }}">
                                    </div>
                                </div>
                                <div class="col-md-6">
                                    <div class="p-3 bg-white border rounded-4 shadow-sm h-100">
                                        ...
                                        <input type="date" name="tmt_gaji_berkala_terakhir" id="tmt_gaji" class="form-control bg-light" 
                                               value="{{ old('tmt_gaji_berkala_terakhir', $pegawai->tmt_gaji_berkala_terakhir) }}">
                                    </div>
                                </div>
                            </div>
...
<script>
    document.addEventListener("DOMContentLoaded", function() {
        ...
        document.getElementById('crop_button').addEventListener('click', function() {
            ...
            // Menghilangkan padding agar foto full
        ...
        function updateGolongan() {
            ...
            if (selected === 'Honorer' || selected === '') {
                ...
                if (selected === 'Honorer') {
                    ...
                    inputTmtPengangkatan.removeAttribute('required'); // Opsional di Edit
                    labelReqTmt.classList.add('d-none');
                } else {
            ...
            if (selected === 'PPPK Paruh Waktu') {
                ...
                labelReqTmt.classList.remove('d-none');
            ...
            // C. JIKA PNS / PPPK BIASA (PENUH WAKTU)
            ...
            // Di Edit, kita tidak paksa required untuk jadwal TMT ini (agar bisa save tanpa mengisi jika memang kosong)
            inputTMTPangkat.removeAttribute('required'); 
            inputTMTGaji.removeAttribute('required');

            // Isi Dropdown Golongan
            if (selected === 'PNS') {
        ...
        // Jalankan saat load awal (Penting untuk mode Edit)
